...
def safe_str(v) -> str:
    """Convert values (including floats/NaN/None) safely to string for len() and Excel widths."""
    if v is None:
        return ""
    try:
        if pd.isna(v):
...
def download_pdf(session: requests.Session, url: str) -> pathlib.Path:
    headers = {
        "User-Agent": UA,
        "Referer": NALCO_URL,
...
def extract_row_ie07(pdf_path: pathlib.Path) -> dict:
    """
...
def to_thousands(value_str: str) -> float | None:
    """Convert raw price (e.g., '268250') to thousands (e.g., 268.250)."""
    value_str = (value_str or "").replace(",", "").strip()
    if not value_str:
        return None
    try:
        v = float(value_str)
...
def save_excel_formatted(df: pd.DataFrame, path: pathlib.Path):
    ...
    # Auto width (safe for numbers/NaN)
...
def safe_append_runlog(log_path: pathlib.Path, info: dict):
    ...
    try:
        ...
        print(f"[WARN] Runlog update failed: {e}", file=sys.stderr)
...
def main():
    ...
    # 5) Convert Basic Price to thousands (e.g., 268250 -> 268.250)
